...
The hazard function under $\lambda=0.5,\,\beta=-0.5$ could be shown as follows:
```{r, echo = FALSE}
...
The hazard function under $\lambda=0.5,\,\gamma=1.2,\,\beta=-0.5$ could be shown as follows:
```{r, echo = FALSE}
...
To observe the potential relevance between test performance and number of samples ($n$), parameter value ($\lambda,\,\gamma$), and coefficient $\beta$, we set $n=50,\,100,\,200$, $\lambda=0,\,0.5,\,1$, $\gamma=1.2,\,1.5$, and $\beta=0,\,1,\,2$. We repeat 50 times for each value setting. The generation process is written as follows:
...
wei_param_df = expand.grid(iteration = c(1:50), n = c(100, 200),
            lambda = c(0.5, 0.8, 1), gamma = c(1.2, 1.5), 
            beta = c(-0.5, -1, -5))
...
ph_exp_df = ph_exp_df |>
  mutate(power = map(simulations, pwr_func)) |>
  unnest(power) |>
  select(-simulations)
kable(ph_exp_df, caption = "Power of 3 Log-Rank Tests based on PH Assumption")

ph_wei_df = ph_wei_df |> 
  mutate(power = map(simulations, pwr_func)) |> 
  unnest(power) |>
  select(-simulations)
kable(ph_wei_df, caption = "Power of 3 Log-Rank Tests based on PH Assumption")

ph_exp_iddf = ph_exp_df |>
  mutate(id = seq_len(nrow(ph_exp_df ))) |>
  select(id, everything()) |>
  pivot_longer(cols = starts_with("test"), names_to = "Test", values_to = "Power") |>
  mutate(Test = str_remove(Test, "_power"))

ph_exp_power_graph = ggplot(ph_exp_iddf, aes(x = id, y = Power, color = Test)) +
  geom_line() +
  ...
  labs(
    x = "Combination id",
    y = "Power",
    title = "Power of Three Tests over combinations of parameters",
    color = "Test"
  )
...
The hazard function under $\lambda_0=0.5,\,\lambda_1=0.3$ could be shown as follows:
```{r, echo = FALSE}
control_func = function(t) return(0.5)
...
kable(late_pw_df, caption = "Power of 3 Log-Rank Tests based on NPH Assumption (Late)")
```
...
kable(early_pw_df, caption = "Power of 3 Log-Rank Tests based on NPH Assumption (Early)")
```
...
The hazard function under $\lambda=0.5,\,\gamma_0=2,\,\gamma_1=1.5$ could be shown as follows:
```{r, echo = FALSE}
...
With the distribution function of survival times, we can write the simulation process as follows.

```{r}
weibull_sim_func = function(n, lambda = 0.5, gamma0, gamma1)
{
  # Set the administrative censoring time to guarantee a censor rate of 0.2 for control arm
  censor_time = qweibull(0.8, shape = gamma0, scale = lambda)
...
kable(late_wei_df, caption = "Power of 3 Log-Rank Tests based on NPH Assumption")
```

If set the survival time for the control arm to follow exponential distribution, and set the hazard function for the treatment arm to be $\lambda$ after reaching $\lambda$, then the hazard function under $\lambda=0.5,\,\gamma=1.5$ could be shown as follows:

```{r, echo = FALSE}
control_func = function(t) return(0.5)
treatment_func = function(t)
  return(0.5 * 1.5 * t ^ (1.5 - 1) * (0.5 * 1.5 * t ^ (1.5 - 1) < 0.5) +
           (0.5 * 1.5 * t ^ (1.5 - 1) >= 0.5) * 0.5)

hazard_plot(control_func, treatment_func, 3,
            "Non-Proportional-Hazard Function for Early-Effect Models")
```

<!-- The hazard function becomes: -->
<!-- $$h(t|x=1)=\left\{ -->
<!-- \begin{aligned} -->
<!-- \lambda & & t\geq \gamma^{-\frac{1}{\gamma-1}} \\ \lambda\gamma t^{\gamma-1} & & t<\gamma^{-\frac{1}{\gamma-1}} -->
<!-- \end{aligned} -->
<!-- \right.$$ -->
<!-- Then, $$H(t|x=1)=\left\{ -->
<!-- \begin{aligned} -->
<!-- \lambda t^\gamma & & t<\gamma^{-\frac{1}{\gamma-1}} \\ \lambda\gamma^{-\frac{\gamma}{\gamma-1}}+\lambda(t-\gamma^{-\frac{1}{\gamma-1}}) & & t\geq \gamma^{-\frac{1}{\gamma-1}} -->
<!-- \end{aligned} -->
<!-- \right.$$ -->
<!-- $$S(t|x=1)=exp(-H(t|x=1))=\left\{ -->
<!-- \begin{aligned} -->
<!-- exp(-\lambda t^\gamma) & & t<\gamma^{-\frac{1}{\gamma-1}} \\ exp(-\lambda\gamma^{-\frac{\gamma}{\gamma-1}}-\lambda(t-\gamma^{-\frac{1}{\gamma-1}})) & & t\geq \gamma^{-\frac{1}{\gamma-1}} -->
<!-- \end{aligned} -->
<!-- \right.$$ -->
<!-- $$F(t|x=1)=1-S(t|x=1)=\left\{ -->
<!-- \begin{aligned} -->
<!-- 1-exp(-\lambda t^\gamma) & & t<\gamma^{-\frac{1}{\gamma-1}} \\ 1-exp(-\lambda\gamma^{-\frac{\gamma}{\gamma-1}}-\lambda(t-\gamma^{-\frac{1}{\gamma-1}})) & & t\geq \gamma^{-\frac{1}{\gamma-1}} -->
<!-- \end{aligned} -->
<!-- \right.$$ -->
<!-- Let $1-U=F(t|x=1)$, then $(1-U)\sim U(0,1)$, $U=S(t|x=1)\sim U(0,1)$. Thus, -->
<!-- $$T=\left\{ -->
<!-- \begin{aligned} -->
<!-- {(-\lambda_^{-1}log(U))}^{1/\gamma} & & U>exp(-\lambda_0) \\ -->
<!-- \frac{\lambda_1-log(U)}{\lambda_0+\lambda_1} & & U\leq exp(-\lambda_0) -->
<!-- \end{aligned} -->
<!-- \right.$$ -->
...
<!-- ```{r} -->
<!-- weiexp_sim_func = function(n, lambda = 1, gamma) -->
<!-- { -->
<!--   # Set the administrative censoring time to guarantee a censor rate of 0.2 for control arm -->
<!--   censor_time = qexp(0.8, rate = lambda) -->
...
<!--   control_df = tibble(x = rep(0, n), t = t0, d = d0, n, lambda, gamma) -->
<!--   treat_df = tibble(x = rep(1, n), t = t1, d = d1, n, lambda, gamma) -->
<!--   return(rbind(control_df, treat_df)) -->
<!-- } -->
<!-- ``` -->
...
<!-- ```{r} -->
<!-- early_wei_param_df =  -->
<!--   expand.grid(iteration = c(1:50), n = c(50, 100, 200),  -->
<!--               lambda = 0.75, gamma = 1.3) -->

<!-- early_wei_results = -->
<!--   mapply(weiexp_sim_func, n = early_wei_param_df$n, -->
<!--          lambda = early_wei_param_df$lambda, gamma = early_wei_param_df$gamma) -->
...
<!-- ```{r} -->
<!-- (early_wei_df =  -->
<!--   early_wei_df |> mutate(power = map(simulations, pwr_func)) |> unnest(power)) -->
<!-- ``` -->
