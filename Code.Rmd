...
ph_exp_df = ph_exp_df |>
  ...
  unnest(power) |>
  select(-simulations)
kable(ph_exp_df, title = "Power of 3 Log-Rank Tests based on PH Assumption")

ph_wei_df = ph_wei_df |> 
  mutate(power = map(simulations, pwr_func)) |> 
  unnest(power) |>
  select(-simulations)
kable(ph_wei_df, title = "Power of 3 Log-Rank Tests based on PH Assumption")
```
...
kable(late_pw_df, title = "Power of 3 Log-Rank Tests based on NPH Assumption (Late)")
```
...
kable(early_pw_df, title = "Power of 3 Log-Rank Tests based on NPH Assumption (Early)")
```
...
```{r}
late_wei_param_df = 
  expand.grid(iteration = c(1:50), n = c(100, 3-00), 
...
kable(late_wei_df, title = "Power of 3 Log-Rank Tests based on NPH Assumption")
```
