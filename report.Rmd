...
This study examines the performance of three hypothesis tests for time-to-event data: the conventional log-rank test and two variants of the weighted log-rank test. The scenarios considered involve both proportional hazard functions, typical of distributions such as the exponential and Weibull, and non-proportional hazard functions, which may exhibit piecewise changes for the exponential distribution and different gamma of Weibull model. We used Monte Carlo simulation techniques to evaluate the power of hypothesis tests for a range of coefficient values.
...
For Weibull distribution with the scale parameter $\lambda$, and is the shape parameter $\gamma$, the possibility density function is $$f_0=\lambda\gamma t^{\gamma-1}exp(-\lambda t^\gamma)$$ Then, \begin{align*}
...
Thus, $$T={(-\lambda^{-1}log(U)exp(-\beta'x))}^{1/\gamma}$$ where $U\sim U(0,1)$.
...
**Late Effect:** For the treatment arm, we suppose the hazard function for the treatment arm is: $$h(t|x=1)=\left\{
\begin{aligned}
...
\end{aligned}
\right.$$ Let $1-U=F(t|x=1)$, then $(1-U)\sim U(0,1)$, $U=S(t|x=1)\sim U(0,1)$. Thus, $$T=\left\{
\begin{aligned}
...
**Early Effect:** We can use the similar simulation method to generate piecewise exponential models in which the treatment arm shows early effect. The hazard function becomes: $$h(t|x=1)=\left\{
\begin{aligned}
...
To simplify the problem, we assume the control and treatment arm share the same scale parameter $\lambda$. For the control arm, suppose the hazard function is: $$h(t|x=0)=\lambda\gamma_0 t^{(\gamma_0-1)}.$$ Then, $$H(t|x=0)=\lambda t^\gamma_0$$ $$S(t|x=0)=exp(-H(t|x=0))=exp(-\lambda t^\gamma_0)$$ $$F(t|x=0)=1-S(t|x=0)=1-exp(-\lambda t^\gamma_0)$$ Let $1-U=F(t|x=0)$, then $(1-U)\sim U(0,1)$, $U=S(t|x=0)\sim U(0,1)$. Thus, $$T={(-\lambda^{-1}log(U))}^{1/\gamma_0}$$
...
**Exponential Distribution:** The hazard function under $\lambda=0.5,\,\beta=-0.5$ could be shown as follows:
...
**Weibull Distribution:** The hazard function under $\lambda=0.5,\,\gamma=1.2,\,\beta=-0.5$ could be shown as follows:
...
**Piecewise Exponential Model:** The hazard function under $\lambda_0=0.5,\,\lambda_1=0.3$ could be shown as follows:
...
Henderson, B. (2014). Special Oxford University Spring School course, June 18-20. Empirical Legal Studies. <https://www.elsblog.org/the_empirical_legal_studi/2007/04/special_oxford_.html>
