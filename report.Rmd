...
**Late Effect:** We suppose the hazard function for the treatment arm is: $h(t|x=1)=\left\{
\begin{aligned}
...
In this scenario, two tables have been presented, each illustrating the baseline function following either an exponential or Weibull distribution. The results of the tables reveals that for assessing the Proportional-Hazard Assumption, the overall log-rank test exhibits superior specificity, indicating more robust results. Furthermore, with an increase in sample size, there is a tendency towards higher specificity rates.
...
In the second scenario of addressing the Non-Proportional-Hazard Assumption, we assume that the treatment group and control group have different shape parameters $\gamma$. In this case, the late weighted log-rank test demonstrates the best performance, consistent with our intuitive findings. Furthermore, increasing the sample size continues to enhance specificity.
